...
def process_data(input_path, Denoise, Normalize, Chunks, Shifts):
    ...
    track = f"{tracks_path}{track}"
    shutil.move(input_path.name, "/content/drive/MyDrive/MDX_Colab/tracks")
    ...
    console(f"python main.py --n_fft {n_fft} --dim_f {dim_f} --dim_t {dim_t} --margin {margin} -i \"{track}\" --mixing {mixing_algorithm} --onnx \"{'onnx/(de)Reverb HQ By FoxJoy'}\" --model off  --shifts {round(shifts)} --stems v --invert v --chunks {chunks} --compensate {amplitude_compensation} {normalise} {denoise}")
    os.remove(track)
    return f'De reverb complete! Please check "/content/drive/MyDrive/MDX_Colab/separated" for your results.'
